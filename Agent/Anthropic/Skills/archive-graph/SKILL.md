...
<skill name="archive-graph" version="2.0.0">
  ...
  <workflow>
    ...
    <step number="2" name="Build Graph Snapshot">
      <command>python Agent\Anthropic\Skills\archive-graph\scripts\build.py --path "[PROJECT_PATH]"</command>
      <output>[PROJECT_PATH]\Agent-Context\Archives\graph.db</output>
    </step>

    <step number="3" name="Query Available Files">
      <command>python Agent\Anthropic\Skills\archive-graph\scripts\query.py --files --project-path "[PROJECT_PATH]"</command>
    </step>

    <step number="4" name="Inspect File Structure">
      <command>python Agent\Anthropic\Skills\archive-graph\scripts\query.py --structure "auth" --project-path "[PROJECT_PATH]"</command>
    </step>

    <step number="5" name="Search Symbol Names">
      <command>python Agent\Anthropic\Skills\archive-graph\scripts\query.py --query "login" --project-path "[PROJECT_PATH]"</command>
    </step>
  </workflow>
