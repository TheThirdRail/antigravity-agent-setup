...
if (-not (Test-Path -Path $SourcePath -PathType Leaf)) {
    Write-Error "Source subagent file not found: $SourcePath"
    exit 1
}
...
switch ($Vendor) {
    'mine' { $RelativeLocalPath = '.agent\subagents' }
    'anthropic' { $RelativeLocalPath = '.claude\agents' }
    'openai' {
        if ($UseLegacyCodexPath) {
            $RelativeLocalPath = '.codex\subagents'
        }
        else {
            $RelativeLocalPath = '.agents\subagents'
        }
    }
    'google' { $RelativeLocalPath = '.gemini\subagents' }
}
...
Write-Host 'Installing workspace subagent' -ForegroundColor Cyan
...
if ($PSCmdlet.ShouldProcess($DestPath, "$ActionName subagent file")) {
    if ($Mode -eq 'Copy') {
