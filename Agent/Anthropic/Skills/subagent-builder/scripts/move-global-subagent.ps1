...
switch ($Vendor) {
    'anthropic' { $GlobalPath = Join-Path $HOME '.claude\agents' }
    'openai' {
        if ($UseLegacyCodexPath) {
            $GlobalPath = Join-Path $HOME '.codex\subagents'
        }
        else {
            $GlobalPath = Join-Path $HOME '.agents\subagents'
        }
    }
    'google' { $GlobalPath = Join-Path $HOME '.gemini\subagents' }
}
...
Write-Host 'Installing global subagent' -ForegroundColor Cyan
