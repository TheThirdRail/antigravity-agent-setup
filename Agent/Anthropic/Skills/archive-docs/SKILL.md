...
<skill name="archive-docs" version="2.0.0">
  ...
  <workflow>
    ...
    <step number="2" name="Add Documents">
      <command>python Agent\Anthropic\Skills\archive-docs\scripts\add.py --id "doc-id" --content "Document text" --metadata "{\"source\":\"docs/architecture.md\"}"</command>
      <command>python Agent\Anthropic\Skills\archive-docs\scripts\add.py --file "docs/architecture.md" --id "architecture"</command>
    </step>

    <step number="3" name="Search Semantically">
      <command>python Agent\Anthropic\Skills\archive-docs\scripts\search.py --query "authentication flow" --limit 5</command>
    </step>

    <step number="4" name="Delete or Replace Stale Entries">
      <command>python Agent\Anthropic\Skills\archive-docs\scripts\delete.py --id "doc-id"</command>
    </step>
  </workflow>
