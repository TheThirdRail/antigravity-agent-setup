...
<skill name="archive-code" version="2.1.0">
  ...
  <workflow>
    <step number="1" name="Search Code">
      <command>.\Agent\OpenAI\Skills\archive-code\scripts\search.ps1 -Pattern "class User" -Path "[PROJECT_PATH]"</command>
      <command>.\Agent\OpenAI\Skills\archive-code\scripts\search.ps1 -Pattern "def login" -Path "[PROJECT_PATH]" -Type "definition"</command>
      <command>.\Agent\OpenAI\Skills\archive-code\scripts\search.ps1 -Pattern "auth_service" -Path "[PROJECT_PATH]" -Context 2</command>
    </step>
  </workflow>
